...
const replaceCJString = (file) => {
    ...
    arrayOfCJCases.forEach(caseString => {
        while (newElement.indexOf(caseString) !== -1) {
            let index = newElement.indexOf(caseString)
...
const replaceExportCJ = (caseString, newString, file, index) => {
    if (caseString.includes('{')) {
        ...
        toRemove = file.substring(index, endExport + 1)

        exportObject = file.substring(startExport + 1, endExport)

        exportValues = exportObject.split(',')

        exportValues.forEach(value => {
            if (value !== '' && value !== '\r\n') {
                if (value.includes(':')) {
                    let variable = value.split(':')
                    let varDefinition = variable[0].replaceAll('"', '')
                    let varValue = variable[1]
...
const replaceExportVariableCJ = (index, file) => {
    ...
    toRemove = file.substring(index, constantIndexEnd)

    toAdd = 'export const ' + file.substring(caseIndexEnd + 1, constantIndexEnd)
...
const replaceImportVariableCJ = (index, caseString, file) => {
    ...
    path = file.substring(startPath, requireEnd)
